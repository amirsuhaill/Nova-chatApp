...
export default function NameModal({ onConfirm, onSkip }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-sm bg-gray-950 border border-violet-800/50 rounded-2xl p-8 shadow-2xl shadow-violet-950/60 flex flex-col items-center text-center msg-enter">
        {/* Avatar */}
        ...
        <h2 className="text-xl font-bold text-white mb-1">Welcome, Explorer</h2>
        ...
        <form onSubmit={handleSubmit} className="w-full flex flex-col gap-3">
          <input
            ...
            maxLength={30}
            className="w-full bg-gray-900/80 border border-violet-800/50 rounded-xl px-4 py-3 text-sm text-gray-200 placeholder-gray-600 outline-none focus:border-violet-500/80 focus:ring-1 focus:ring-violet-500/30 transition-all text-center"
          />
          <button
            type="submit"
            disabled={!value.trim()}
